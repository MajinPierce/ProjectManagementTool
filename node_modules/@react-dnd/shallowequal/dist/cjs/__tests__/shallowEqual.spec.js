...
describe('shallowequal', function() {
    ...
    it('returns false if either argument is null', ()=>{
        expect((0, _index).shallowEqual(null, {})).toEqual(false);
        expect((0, _index).shallowEqual({}, null)).toEqual(false);
    });
    it('returns true if both arguments are null or undefined', ()=>{
        expect((0, _index).shallowEqual(null, null)).toEqual(true);
        expect((0, _index).shallowEqual(undefined, undefined)).toEqual(true);
    });
    it('returns true if arguments are shallow equal', ()=>{
        expect((0, _index).shallowEqual({
            a: 1,
            b: 2,
            c: 3
        }, {
    ...
    it('returns false if arguments are not objects and not equal', ()=>{
        expect((0, _index).shallowEqual(1, 2)).toEqual(false);
    });
    it('returns false if only one argument is not an object', ()=>{
        expect((0, _index).shallowEqual(1, {})).toEqual(false);
    });
    ...
    it('should provide the correct `customizer` arguments', ()=>{
        ...
        (0, _index).shallowEqual(object1, object2, function(...args) {
            argsList.push(args);
        });
        expect(argsList).toEqual(expected);
    });
    it('should set the `this` binding', ()=>{
        const actual = (0, _index).shallowEqual('a', 'b', function compare(a, b) {
    ...
    it('should handle comparisons if `customizer` returns `undefined`', ()=>{
        const noop = ()=>void 0
        ;
        expect((0, _index).shallowEqual('a', 'a', noop)).toEqual(true);
        expect((0, _index).shallowEqual([
            'a'
        ], [
            'a'
    ...
    it('should not handle comparisons if `customizer` returns `true`', ()=>{
        ...
        ;
        expect((0, _index).shallowEqual('a', 'b', customizer)).toEqual(true);
        expect((0, _index).shallowEqual([
            'a'
        ], [
            'b'
    ...
    it('should not handle comparisons if `customizer` returns `false`', ()=>{
        ...
        ;
        expect((0, _index).shallowEqual('a', 'a', customizer)).toEqual(false);
        expect((0, _index).shallowEqual([
            'a'
        ], [
            'a'
    ...
    it('should return a boolean value even if `customizer` does not', ()=>{
        const actual = (0, _index).shallowEqual('a', 'b', ()=>'c'
        );
        expect(actual).toEqual(true);
        ...
        values.forEach((value)=>{
            result.push((0, _index).shallowEqual('a', 'a', ()=>value
            ));
        });
    ...
    it('should treat objects created by `Object.create(null)` like any other plain object', ()=>{
        ...
        expect((0, _index).shallowEqual(new Foo(), object2)).toEqual(true);
        const object1 = Object.create(null);
        object1.a = 1;
        expect((0, _index).shallowEqual(object1, object2)).toEqual(true);
    });
});
